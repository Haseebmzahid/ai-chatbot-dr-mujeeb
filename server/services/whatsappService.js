...
import { compactText, makePublicId, normalizePhone } from "../utils/time.js";
...
export async function logMessage({
  ...
  try {
    return await models.MessageLog.create({
      ...
      appointmentId,
      messageType,
      messageBody: compactText(messageBody, 1000),
      direction,
      status,
      providerMessageId,
...
function buildWhatsAppPayload({ normalizedPhone, text, useTemplate, templateName, templateComponents, language }) {
  const base = {
...
export async function sendWhatsAppText({
  ...
  try {
    ...
    await logMessage({
      ...
      rawPayload: result.ok ? undefined : result.body
    });
...
export async function listMessageLogs(limit = 300) {
  return models.MessageLog.find({}).sort({ createdAt: -1 }).limit(Math.min(Number(limit) || 300, 1000)).lean();
}

export async function updateMessageStatus(providerMessageId, status, rawPayload = null) {
